...
// ── "and" entries ──────────────────────────────────────────────────────────
// Settings lets Trevor say "send it to Wiring only when the description has
// BOTH install AND pickup". He types two ordinary words; this turns them into
// ONE stored keyword that the matcher above already understands, so no new
// matching mechanism exists and nothing else in inferBench changed.
//
// Two things make it work:
//  1. It is stored QUOTED, so it goes through the priority pass — that is what
//     lets it beat bench order. Unquoted it could never win: Electronics is
//     tested before Wiring and already owns 'pickup', so a plain 'install' on
//     Wiring moved ten jobs to Electronics and Admin and none to Wiring when
//     Trevor tried it live on 2026-09-01.
//  2. The pattern deliberately CONSUMES one of the two words. The priority pass
//     scores hits by `m[0].length` and skips an empty match, so a pure
//     both-present test would be found and then silently thrown away. The
//     trailing `(?:a|b)` makes the match the first of the two words to appear,
//     which is a sensible, predictable length to score.
//
// Trevor's words are escaped, so a word containing a dot, bracket or plus is
// matched literally and cannot break the pattern.

// Every character that means something in a pattern, made literal.
export function escapeKeywordWord(word) {
  return String(word ?? '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

const AND_PREFIX = '(?=[\\s\\S]*';
// One escaped word: backslash-pairs stay glued together, so a word containing
// an escaped ')' can never be mistaken for the end of the group.
const AND_WORD = '((?:\\\\[\\s\\S]|[^\\\\])+?)';
const AND_SHAPE = new RegExp(
  '^\\(\\?=\\[\\\\s\\\\S\\]\\*' + AND_WORD + '\\)' +
  '\\(\\?=\\[\\\\s\\\\S\\]\\*' + AND_WORD + '\\)' +
  '\\(\\?:' + AND_WORD + '\\|' + AND_WORD + '\\)$'
);

// Two plain words in, one stored keyword out. Returns null if either word is
// blank, so a half-filled box stores nothing.
export function buildAndKeyword(a, b) {
  const w1 = String(a ?? '').trim().toLowerCase();
  const w2 = String(b ?? '').trim().toLowerCase();
  if (!w1 || !w2) return null;
  const e1 = escapeKeywordWord(w1);
  const e2 = escapeKeywordWord(w2);
  return `"${AND_PREFIX}${e1})${AND_PREFIX}${e2})(?:${e1}|${e2})"`;
}

// The reverse: a stored keyword back to the two words, or null when it is an
// ordinary keyword. This is what the Settings chip reads, so Trevor sees
// `install + pickup` and never the stored pattern.
export function andKeywordWords(word) {
  const inner = quotedKeyword(word);
  if (!inner) return null;
  const m = inner.match(AND_SHAPE);
  if (!m) return null;
  const unescape = t => t.replace(/\\([\s\S])/g, '$1');
  const w1 = unescape(m[1]);
  const w2 = unescape(m[2]);
  // Only trust it if the consuming half is the same pair — anything else is a
  // pattern we did not write and is left alone as an ordinary keyword.
  if (unescape(m[3]) !== w1 || unescape(m[4]) !== w2) return null;
  return [w1, w2];
}
