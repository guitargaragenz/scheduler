...
// Brief G, Build 1b — the six fields that moved from the CSV/Sheet to the app.
//
// Tag, Hours, Action, VB, BL and PJ are Trevor's own judgement calls, not
// facts Multitrack reports. Multitrack is the authority on job number,
// customer, mfr, model, status and desc; nothing else on a job comes from
...
//
//  1. That constant feeds jobsStateFieldsFor(), so every scheduling, pomodoro
//     and mark-done write would start carrying tag/hours/action/vb/bl/pj as
//     extra columns. Dragging a job onto the calendar has no business
//     restating its hours.
//  2. Worse, it also feeds pickTopLevelState(), which expandAutoSplits()
//     applies to the STORED row of a derived auto-split bench card. A derived
//     card's hours are per-session and regenerated by createSubtasks() on
//     every load; letting a stored hours value cross over there pins a stale
//     figure permanently — the exact corruption the long note in
//     joinJobsMasterState() below exists to prevent.
//
// What the ownership move needs is the opposite of that list: these fields
// must stop being written by the CSV path, which is NON_MASTER_FIELDS'
// job, not JOBS_STATE_TOP_LEVEL_FIELDS'.
export const APP_OWNED_JOB_FIELDS = Object.freeze([
