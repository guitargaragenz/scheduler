...
export default function Sidebar({ jobs, dragMode, onDragModeChange, onPdfUpload, highlightedJobId, onClearHighlight, onJobClick, isOpen, onToggle, lastSyncedAt, focusList = [], onToggleFocus }) {
  ...
  if (isFocusMode) {
    ...
    const matchText  = j => [j.job, j.customer, j.mfr, j.model, j.bench, j.desc, j.status, j.action]
      .some(v => String(v || '').toLowerCase().includes(q));
    const SUB_ONLY_BENCHES = new Set(['Wiring', 'Finishing']);
    const matchBench = j => !benchFilter || j.bench === benchFilter ||
      (SUB_ONLY_BENCHES.has(benchFilter) && jobs.some(sub => sub.parentId === j.id && sub.bench === benchFilter));
    const matchHours = j => {
            ...
            {isFocusMode ? (
              ...
              <>
                ...
                <input
                  ...
                  placeholder="Job #, customer, make, model, bench…"
                  style={{
