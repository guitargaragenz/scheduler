...
Spawn all 5 advisors simultaneously as sub-agents, **each with `model: "sonnet"` set explicitly**. Do not let them inherit the session model — a council run is 11 sub-agents, and inheriting Opus makes one `council this` cost more than a day of normal work. The council's value comes from five *independent* angles, not five expensive ones.
...
Spawn 5 new sub-agents, one for each advisor, **again with `model: "sonnet"` set explicitly**. Each reviewer sees all 5 anonymized responses and answers three questions:
...
**Do the chairman synthesis yourself, in the main conversation — do not spawn an eleventh sub-agent for it.** You already hold the question, the context and all ten responses; a fresh agent would have to be re-fed every one of them. This is the one step where the main session is both the cheapest option and the best-informed one.
...
- **Always set `model` explicitly on every sub-agent spawn — `sonnet` for advisors and reviewers.** Never let them inherit. This is enforced by a hook in the GGNZ project and it will block the spawn if you forget.
