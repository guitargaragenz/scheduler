...
- [ ] **Desktop "Place on Calendar" drawer bug (2026-06-17)** — still unverified. Today's Remove-from-Calendar fix touched the same PomoDrawer area but was a different action (unschedule, not schedule) — does not confirm this is fixed. Re-test before closing.
...
- [ ] **Day-view job-shelf `minWidth: 700` overflow** — `CalendarGrid`'s inner container has a hardcoded `minWidth: 700`, which is wider than the Daily Log's schedule column (~260-300px), causing job cards to render partially off the visible column and require horizontal scroll. Not yet fixed — noticed during drag-testing today. Was flagged as a risk in the original single-day-calendar plan but not resolved.
...
- [ ] **Restart `start_watcher.command` on Micky** — paused during the investigation/fix, needs to be manually restarted once Trevor's ready.
