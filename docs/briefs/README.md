...
|-------|------|---------------|
| [`.claude/pending-brief.md`](../../.claude/pending-brief.md) | 2026-07-29 | **Current — start here. Brief H, Build 2: retire the CSV pipeline.** ✅ **Approved by Trevor, and council is done** — two reviewers, both yay, both corrections folded in. **Resumes at step 3, Build 2a, on the existing empty branch `build-2a-close-csv-door`** (cut from the tip of `main`). Read the brief's "Session state" section first: 2a is built **inline, no `ggnz-builder` agent** — a builder spawn hit a monthly spend limit, so Trevor moved 2a to a fresh session to keep token use down. The independent verifier is **not** waived. 2a closes the app's Upload CSV door (`handleCsvUpload`, `parseCSV`, `RAW_CSV`, three upload buttons, the dead help article); 2b drops the app's use of the stored `days` number; 2c deletes the Mac-side watcher/Sheet scripts. The `days` **database column stays** — Trevor's call, don't re-open it. **This is the last item under the standing order — UI work unblocks when it ships.** |
