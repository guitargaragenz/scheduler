...
| Brief | Date | Status |
|-------|------|--------|
| [`.claude/pending-brief.md`](../../.claude/pending-brief.md) | 2026-08-13 | **Bench view Build 1b — add a job to the week page. Council is DONE (2026-08-13, two reviewers) — do not re-run it. Next step is the builder on a staging branch.** Council's one blocking finding is already resolved in the scope lock: the week table can only store a mark on a real day, so "adding a job" had no legal way to leave the row blank. Trevor's call: an added row shows **nothing** until he taps days himself, held by a week-scoped key that isn't one of the seven day keys, so it draws and exports blank with no new marker symbol. Also written in: "already a row" is checked by job id across the week (not per bench group), removal is one labelled action on the row, and jobs with no bench can't be added this way. Blast radius confirmed clean — `bench_week_marks` only. |
