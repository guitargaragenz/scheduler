...
|-------|------|---------------|
| [re-fresh-brief-g-checkpoint-3b-and-build-1b.md](re-fresh-brief-g-checkpoint-3b-and-build-1b.md) | 2026-07-29 | **Current — start here.** Brief G's **Build 1a is shipped** (`f927248`) — the Multitrack PDF now imports into the app, six PDF fields only, Trevor's five hand-kept fields untouched and proved untouched against live data. **Next is checkpoint 3b, which only Trevor can clear**: the database's Tag/Hours/Action/VB/BL for all 53 top-level jobs checked against the Google Sheet, because Build 1b makes the app master and freezes whatever is there. The table is already pulled into the brief. **Build 1b (items 3, 4, 4b — ownership move, Jobs Sheet page, M/T swap fix) does not start until he clears it.** The scope-locked instruction set is still `.claude/pending-brief.md`. |
