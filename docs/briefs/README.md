...
| Brief | Date | Status |
|-------|------|--------|
| [`.claude/pending-brief.md`](../../.claude/pending-brief.md) | 2026-08-08 | **Current — start here.** Scope lock for **revenue Build 3: load what's already saved.** Nothing in the app calls `loadCompletedJobs()`, so the Board's week revenue reads $0 after a page reload — the data is safe in `completed_jobs`, it's just never read back. Wires the bounded reader + subscription in on app start. **Council complete 2026-08-08 (both reviewers, changes folded in at `e281471`). Resumes at protocol step 3 (builder) once Trevor says "yp".** Handoff: [HANDOFF-2026-08-08-revenue-build-3.md](HANDOFF-2026-08-08-revenue-build-3.md). Council's reasoning (background only): [revenue-build-3-council.md](revenue-build-3-council.md). Previous occupant of this file (revenue Builds 1–2, closed at `60ff7bf`): **This file is the scope lock for the revenue brief** (approved by Trevor 2026-08-07, council complete). It is the same work as the row below — read that row for where the build actually stands. Corrected 2026-08-08: this row previously described the blocked-work brief, whose Build 2 (the read-only Bench board) shipped 2026-08-05 at `7baa884` (PR #22). That work is finished; the scope lock moved on to revenue. |
| [revenue-data-loss-fix.md](revenue-data-loss-fix.md) | 2026-08-07 | **Both builds shipped. Build 1 `c9be008` (2026-08-07), Build 2 `60ff7bf` (2026-08-08).** Build 2 bounded every `completed_jobs` read to the weeks it needs — the Sunday export no longer downloads the whole history. Verified 6/6, 604 tests green, totals unchanged (20 rows / $3,295.26 ex-GST). **Left open, needs its own brief:** nothing in the app ever calls `loadCompletedJobs()`, so the Board's week revenue reads $0 after a page reload — pre-existing, not introduced by either build. Old text follows. Build 1: Build 1: the app can no longer delete a revenue line. `clearCompletedJobs()` and `saveCompletedJobs()` are gone; `appendCompletedJob()` writes one row keyed on the **top-level** job (split work is invoiced combined, so `parentId` is walked up first); a second write is refused *and reported*; every failure raises a toast. The live null `job_number` was repaired by `scripts/repair_completed_job_numbers.mjs`. **Build 2 — the twelve files that read `completed_jobs` and assume the whole list is in memory — has not started.** The table now grows unbounded. Section 4 of the brief lists them. **The backfill is done** — see the 2026-08-08 handoff below. |
