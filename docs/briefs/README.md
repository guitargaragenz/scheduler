...
|-------|------|---------------|
| [re-fresh-brief-g-checkpoint-3b-and-build-1b.md](re-fresh-brief-g-checkpoint-3b-and-build-1b.md) | 2026-07-29 | **Current — start here.** Brief G's **Build 1a is shipped** (`f927248`) — the Multitrack PDF now imports into the app, six PDF fields only, Trevor's hand-kept fields untouched and proved untouched against live data. **Checkpoint 3b is CLEARED (2026-07-29)** — Trevor checked the database's Tag/Hours/Action/VB/BL/**PJ** for all 53 top-level jobs against the Google Sheet and signed them off; it matched as-is, no CSV sync needed. The one fix was job `1671`'s bad Action (`parts`), blanked directly in Supabase — that job is closed and not in the Sheet at all, so the write is durable. **Widened 2026-07-29 from five fields to six** — the Sheet owns eight columns, not five, and `PJ` had been missed by every earlier draft; `Days` turned out to belong to a new **Build 1c** instead (a second PDF drop for job dates, so age computes live rather than going stale). **Build 1b is unblocked and not started — the next session builds it; there is a START HERE handoff block at the top of the brief.** The scope-locked instruction set is still `.claude/pending-brief.md`; its 2026-07-29 amendment was approved by Trevor the same day ("yp"), so nothing is waiting on him. |
