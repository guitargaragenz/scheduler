...
|-------|------|---------------|
| [`.claude/pending-brief.md`](../../.claude/pending-brief.md) | 2026-07-29 | **Current — start here. Brief H, Build 2: retire the CSV pipeline.** ⏳ **Draft, awaiting Trevor's "yp" — council has not seen it.** Closes the app's Upload CSV door (`handleCsvUpload`, `parseCSV`, three upload buttons), removes the stored `days` column and `preserveKnownDays()` behind a live-data gate, and deletes the Mac-side watcher/Sheet scripts. Proposed as 2a/2b/2c; council rules on whether that split is right, on whether the `days` database column is dropped or merely abandoned, and on anything still depending on the CSV path. **This is the last item under the standing order — UI work unblocks when it ships.** |
