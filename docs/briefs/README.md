...
| Brief | Date | Status |
|-------|------|--------|
| [bench-view.md](bench-view.md) | 2026-08-13 | **Background only** for the bench-view build — the binding scope lock is `.claude/pending-brief.md`. Trevor is parking automatic scheduling (not deleting it) and moving to his real bullet-journal spread: a week page of job names grouped by bench with M–S day columns plus a trailing `>`, marked `·` booked, `/` worked, `>` move on, `×` done (an `×` auto-`×`s the trailing column and strikes the row through); and a day page with Google Calendar appointments pulled in read-only, typed tasks, and job splits Trevor picks by typing the job number. Approved 2026-08-13. **Next: council in a fresh session, then the build in another.** |
