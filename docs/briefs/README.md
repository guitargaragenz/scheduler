...
|-------|------|---------------|
| [re-fresh-brief-g-checkpoint-3b-and-build-1b.md](re-fresh-brief-g-checkpoint-3b-and-build-1b.md) | 2026-07-29 | **Current — start here.** Brief G's **Build 1a is shipped** (`f927248`) — the Multitrack PDF now imports into the app, six PDF fields only, Trevor's hand-kept fields untouched and proved untouched against live data. **Next is checkpoint 3b, which only Trevor can clear**: the database's Tag/Hours/Action/VB/BL/**PJ** for all 53 top-level jobs checked against the Google Sheet, because Build 1b makes the app master and freezes whatever is there. The table is already pulled into the brief. **Widened 2026-07-29 from five fields to six** — the Sheet owns eight columns, not five, and `PJ` had been missed by every earlier draft; `Days` turned out to belong to a new **Build 1c** instead (a second PDF drop for job dates, so age computes live rather than going stale). **3b is NOT cleared** — five of the six fields are done, `PJ` is still outstanding, and `1671`'s bad Action still has to be blanked in the Sheet as well as the database. **Build 1b does not start until it is.** The scope-locked instruction set is still `.claude/pending-brief.md`, amended 2026-07-29 and needing a fresh "yp". |
