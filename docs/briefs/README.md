...
| Brief | Date | Status |
|-------|------|--------|
| [`.claude/pending-brief.md`](../../.claude/pending-brief.md) | 2026-08-07 | **This file is the scope lock for the revenue brief** (approved by Trevor 2026-08-07, council complete). It is the same work as the row below — read that row for where the build actually stands. Corrected 2026-08-08: this row previously described the blocked-work brief, whose Build 2 (the read-only Bench board) shipped 2026-08-05 at `7baa884` (PR #22). That work is finished; the scope lock moved on to revenue. |
| [revenue-data-loss-fix.md](revenue-data-loss-fix.md) | 2026-08-07 | **Build 1 shipped 2026-08-07 (`c9be008`) — Build 2 is the live work.** Build 1: the app can no longer delete a revenue line. `clearCompletedJobs()` and `saveCompletedJobs()` are gone; `appendCompletedJob()` writes one row keyed on the **top-level** job (split work is invoiced combined, so `parentId` is walked up first); a second write is refused *and reported*; every failure raises a toast. The live null `job_number` was repaired by `scripts/repair_completed_job_numbers.mjs`. **Build 2 — the twelve files that read `completed_jobs` and assume the whole list is in memory — has not started.** The table now grows unbounded. Section 4 of the brief lists them. **The backfill is done** — see the 2026-08-08 handoff below. |
| [handoff-revenue-backfill-and-invoices-pdf.md](handoff-revenue-backfill-and-invoices-pdf.md) | 2026-08-08 | **Build 1 proven live with real data** (321/592/1714 reintroduced and ticked; the existing row survived). Revenue backfilled and reconciled against Multitrack — 20 rows, $3,295.26, all ex-GST. Six one-off `scripts/` document what was changed. **New: Trevor wants an Invoices PDF import so the record covers the financial year from 1 Apr.** That importer does not exist and is blast-radius work needing the full protocol. Key finding that makes it possible: revenue screens never join back to `jobs`, so a revenue row does not need a live job behind it. |
| [handoff-2026-08-08-invoices-brief-and-revenue-tidy.md](handoff-2026-08-08-invoices-brief-and-revenue-tidy.md) | 2026-08-08 | **Current — start here.** The Invoices import brief is written and rewritten to Trevor's scope lock (*no invoice data in the database, at all*) — dedupe is now a date cutoff, no new column. **Awaiting his "yp"; it has not been to council.** Still blocked on a real List Invoices PDF sample. Also open: job 1712 missing make/model, and hours on revenue rows being estimates not worked time (cosmetic — nothing totals them). Both are `scripts/` jobs, never hand-edits. |
| [brief-invoices-pdf-import.md](brief-invoices-pdf-import.md) | 2026-08-08 | The brief itself. Protocol step 1, **not yet approved**. |
