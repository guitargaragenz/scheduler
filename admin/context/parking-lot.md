...
- [ ] **Google Calendar still shows stale event blocks for ghost jobs** #1520/#1582/#1647/#1681/#1699 (from the 2026-07-04 ghost-slot cleanup) — cleaning Firestore's `scheduledSlots` didn't touch GCal, since that cleanup bypassed `unscheduleJob()`'s own `deleteEvent()` call. The split-child job records with their `gcalEventId`s are already gone from `jobs[]`, so there's no way to programmatically identify which GCal events to delete anymore. Trevor needs to manually delete these blocks directly in Google Calendar.
- [ ] **#1520 (Ampeg SVT 6 Pro, Pete Johanson) and #1582 (Roland Juno 106, Jason Crawford) need manual re-split + re-schedule** — their scheduled state was lost before the 2026-07-04 `sheet_to_csv.command` fix existed; the fix stops new damage but doesn't restore old data. Both still have real, active job data.
